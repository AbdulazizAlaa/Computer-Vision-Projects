...
void getLines(Mat & detEdges, Mat & src){
    vector<Vec4i> lines;
    HoughLinesP(detEdges, lines, 1, CV_PI/theta, houghThresh, minLineLength, 10 );
    Mat clone = src.clone();
...
void CannyThreshold(int, void*)
{
  ...
  GaussianBlur( src_gray, detected_edges, Size(5,5), sigma);
...
int main( int argc, char** argv )
{
  ...
  /// Create a Trackbar for user to enter threshold
  createTrackbar( "theta:", window_name, &theta, max_theta, CannyThreshold );
